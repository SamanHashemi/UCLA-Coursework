...
window.Scoreboard = window.classes.Scoreboard = 
class Scoreboard extends Scene_Component
{ 
  constructor( context, control_box, s1, s2, canvas = context.canvas)
  { super( context, control_box );
    this.context = true;
    this.reset = false;
    this.s1 = s1;
    this.s2 = s2; //need to dynamically update score in Main_Scene
    this.wins1 = 0;
    this.wins2 = 0;
    //add win condition variables -> should be easy
  }
  show_explanation( document_element ) { }
  make_control_panel()
  {
    this.live_string( box => box.textContent = "Player 1: " + this.s1);
    this.new_line();        // The facing directions are actually affected by the left hand rule: 
    this.live_string( box => box.textContent = " Player 2: " + this.s2 );
    this.new_line();
    this.live_string( box => box.textContent = "Set Score Player 1: " + this.wins1 );
    this.live_string( box => box.textContent = "Set Score Player 2: " + this.wins2 );
    this.new_line();
    this.key_triggered_button( "Reset Game Score", [ "g" ], () => {this.s1 = 0, this.s2=0}, undefined, () => {this.s1 = 0, this.s2=0}); //change the keys later
    this.key_triggered_button( "New Game", [ "t" ], () => {this.wins1 = 0, this.wins2=0, this.s1 = 0,this.s2=0}, undefined, () => {this.wins1 = 0, this.wins2=0, this.s1 = 0,this.s2=0}); //change the keys later
    //Maybe a best of series?
  }
  updateScore1() {
    this.s1 = this.s1+1;      
    if(this.s1 == 25){
...
{                                                  // other Scene, but it is a Secondary Scene Component -- meant to stack alongside other
  ...
  make_control_panel()                                                        // This function of a scene sets up its keyboard shortcuts.
    { const globals = this.globals;
      ...
      this.key_triggered_button( "Forward",[ "1" ], () => this.thrust[2] =  1, undefined, () => this.thrust[2] = 0 );  this.new_line();
      this.key_triggered_button( "Left",   [ "2" ], () => this.thrust[0] =  1, undefined, () => this.thrust[0] = 0 );
      this.key_triggered_button( "Back",   [ "3" ], () => this.thrust[2] = -1, undefined, () => this.thrust[2] = 0 );
      this.key_triggered_button( "Right",  [ "4" ], () => this.thrust[0] = -1, undefined, () => this.thrust[0] = 0 );  this.new_line();
