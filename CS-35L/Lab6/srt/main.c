...
// Array for the values of the thread (tbp)
float threadVals[width][height][3];

/* The multithreading function */
void * threader(void * num_threads){
  Vec3 camera_pos;
  ...
  const double pixel_dy = tan( 0.5*camera_fov ) / ((double)height*0.5);
    ...
    /* for every pixel */
    for( int px=*(int*)num_threads; px<width; px+=nthreads)
      {
        ...
        for( int py=0; py<height; ++py )
	  {
	      ...
	      scaled_color[i] = max( min(scaled_color[i], 255), 0);
            ...
            // Assign the correct values to the threadVals;
            for( int i=0; i<3; i++){
	      threadVals[px][py][i] = scaled_color[i];
            }
	  }
      }
    return NULL;
...
main( int argc, char **argv )
{
  ...
  // No longer needed because multi-threading works!
  //    if( nthreads != 1 )
  //    {
  ...
  int thread_val[nthreads];
    
  // Set the values of the threads
  for(int i=0; i<nthreads; i++){
    thread_val[i] = i;
    pthread_create(&threads[i], NULL, threader,&thread_val[i]);
  }
